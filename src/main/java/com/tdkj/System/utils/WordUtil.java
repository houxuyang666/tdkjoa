...
import com.tdkj.System.common.CustomXWPFDocument;
...
public class WordUtil {
                            ...
                            if (text.indexOf(key) != -1) {
                                ...
                                if (value instanceof String) {//文本替换
                                    if (value.equals("null")||value.equals("")){
                                        ...
                                        text = text.replace(key, "该项目暂不需要监测该关键点");
                                        continue;
                                    }
                                    ...
                                    Map pic = (Map) value;
                                    if (pic.get("content").equals("null")||pic.get("content").equals("")){
                                    ...
                                    System.out.println((String) pic.get("content"));
                                    //获取图片流，因本人项目中适用流
                                    //InputStream is = (InputStream) pic.get("content");
                                    String byteArray = (String) pic.get("content");
                                    //System.out.println(byteArray+"byteArray");
                                    String [] picParam=byteArray.split(";");
                                    for (int i = 0; i < picParam.length; i++) {
                                        // System.out.println(picParam[i]+"picParam[i]");
                                        CTInline inline = run.getCTR().addNewDrawing().addNewInline();
                                        insertPicture(doc, picParam[i], inline, width, height,picType);
                                    }
                                }
                            }
                        }
    ...
    private static void insertPicture(XWPFDocument document, String filePath,
        ...
        // System.out.println(id+"document.getAllPictures().size() - 1");
        final int EMU = 9525;
        width *= EMU;
        height *= EMU;
        String blipId =
